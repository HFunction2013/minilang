...
# Boot compiler (C) compiles compiler.ml -> bytecode text A
# Self-hosted compiler (compiler.ml running on VM) compiles compiler.ml -> bytecode text B
...
echo ""
echo "=== Step 1: Boot compiler compiles compiler.ml ==="
./minilang dump-text compiler.ml > /tmp/minilang_boot_bc.txt 2>/tmp/minilang_boot_err.txt
echo "Exit code: $?"
...
echo ""
echo "=== Step 2: Self-hosted compiler compiles compiler.ml ==="
echo "(compiler.ml runs on VM, reads its own source from stdin)"
cat compiler.ml | ./minilang run compiler.ml > /tmp/minilang_self_bc.txt 2>/tmp/minilang_self_err.txt
echo "Exit code: $?"
